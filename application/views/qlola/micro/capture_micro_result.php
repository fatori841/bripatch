					...
					foreach($micro_list as $key => $d){
						...
						<tr>
							...
							<td align="right">
								...
								if($d->capture_status == 0){
								?>
								<a class="btn btn-info btn-sm btn_download_capture" id="btn_download_capture_<?php echo $d->id; ?>" capture_id="<?php echo $d->id; ?>" href="<?php echo site_url('Qlola/download_capture_micro').'/'.$d->id;?>">
...
$(document).ready(function(){
	/*$('.btn_remove_micro').click(function(){
		...
		if(confirm('Are you sure want to delete '+micro_name+'?')){
			$.ajax({
				type : "POST",
				url  : "<?=site_url('Qlola/delete_micro');?>",
