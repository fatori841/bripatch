			...
			<?php
				if(isset($data_tbl_bank_brimo)){
					foreach($data_tbl_bank_brimo as $key => $d){
						if($d->is_bifast == 1 && $d->is_bifast_problem == 0 ){
							$text_btn = "Terbuka"; 
							$button_color = "blue";
							} else if ($d->is_bifast == 1 && $d->is_bifast_problem == 1){
								$text_btn = "Tertutup";
								$button_color = "red";
							} else {
								$text_btn = "Tertutup";
...
$(document).ready(function(){
	$('.btn_run').click(function(){
		...
		$.ajax({
			type : "POST",
			url  : "<?=site_url('Brimo/buka_tutup_bifast_satuan_v3');?>",
			...
			success: function(data){
				...
				if(window.confirm("Apakah Bricams Addons dibuka / ditutup juga?")){
