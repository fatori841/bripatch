<div class="pagetitle">
  <h1 align="center">BRIMO - Open Close BIFAST V3 All</h1>
</div>
<!-- End Page Title -->
...
$(document).ready(function(){
	$('#search_data_bifast').submit(function(){
		$.ajax({
			type : "POST",
			url  : "<?=site_url('Brimo/show_bifast_result_v3');?>",
			data : $(this).serialize(),
			beforeSend:function(){
				//$('#ajax-loader').show();
...
$(document).ready(function(){
	$('#close_bifast').submit(function(){
		$.ajax({
			type : "POST",
			url  : "<?=site_url('Brimo/close_bifast_all_v3');?>",
			data : $(this).serialize(),
			beforeSend:function(){
				//$('#ajax-loader').show();
			...
			success: function(data){
				...
				$('#close_bifast_btn').text("CLOSE BIFAST");
				$('#form_result').html(data);
				if(window.confirm("Apakah Bricams Addons ditutup juga?")){
					window.location.href = "../bricams/show_bifast_addons";
				}
			}
		});
		return false;
...
$(document).ready(function(){
	$('#open_bifast').submit(function(){
		$.ajax({
			type : "POST",
			url  : "<?=site_url('Brimo/open_bifast_v3');?>",
			data : $(this).serialize(),
			beforeSend:function(){
				//$('#ajax-loader').show();
			...
			success: function(data){
				...
				$('#open_bifast_btn').text("OPEN BIFAST");
				$('#form_result').html(data);
				if(window.confirm("Apakah Bricams Addons dibuka juga?")){
					window.location.href = "../bricams/show_bifast_addons";
				}
			}
		});
