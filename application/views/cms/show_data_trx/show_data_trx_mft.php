<div class="card mb-3">
	...
	<div class="card-body">
	  ...
	  <form id="search_data_trx_cft" class="row g-3 needs-validation" action="" method="post" novalidate>
		<div class="row mb-3">
		  ...
		  <div class="col-sm-10">
			<select class="form-select" aria-label="Default select example" name="search_by" id="search_by" required>
			  <option selected value="search_by_parentid">ID Parent MFT</option>
			  <option selected value="search_by_detailid">ID Detail MFT</option>
			  ...
			  <option value="search_by_last_cft_client">10 Transaksi Terakhir Collective FT</option>
		...
		<div class="col-12">
		  <button class="btn btn-primary w-100" type="submit" id="search_cft_btn">
			Search
		  </button>
		</div>
	  </form>
...
$(document).ready(function(){
	...
	$('#search_data_trx_cft').submit(function(){
		$.ajax({
			type : "POST",
			url  : "<?=site_url('Cms/search_data_trx_cft');?>",
			data : $(this).serialize(),
			beforeSend:function(){
				$('#search_cft_btn').attr("disabled","disabled");
				$('#search_cft_btn').text('Loading...');
				$('#search_cft_btn').prepend('<span class="spinner-border spinner-border-sm" role="status" style="margin-right:5px;" aria-hidden="true"></span>');
			},
			error: function(){
				$('#search_cft_btn').removeAttr("disabled");
				$('#search_cft_btn').text("Search");
