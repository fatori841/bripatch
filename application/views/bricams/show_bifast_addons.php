<div class="pagetitle">
  <h1 align="center">Bricams Addons - Open Close BIFAST All</h1>
</div>
<!-- End Page Title -->
...
$(document).ready(function(){
	$('#search_data_bifast').submit(function(){
		$.ajax({
			type : "POST",
			url  : "<?=site_url('Bricams/show_bifast_addons_result');?>",
			data : $(this).serialize(),
			beforeSend:function(){
				//$('#ajax-loader').show();
...
$(document).ready(function(){
	$('#close_bifast').submit(function(){
		$.ajax({
			type : "POST",
			url  : "<?=site_url('Bricams/close_bifast_all_addons');?>",
			data : $(this).serialize(),
			beforeSend:function(){
				//$('#ajax-loader').show();
			...
			success: function(data){
				...
				$('#close_bifast_btn').text("CLOSE BIFAST");
				$('#form_result').html(data);
				if(window.confirm("Apakah ibbiz ditutup juga?")){
...
$(document).ready(function(){
	$('#open_bifast').submit(function(){
		$.ajax({
			type : "POST",
			url  : "<?=site_url('Bricams/open_bifast_addons');?>",
			data : $(this).serialize(),
			beforeSend:function(){
				//$('#ajax-loader').show();
			...
			success: function(data){
				...
				$('#open_bifast_btn').text("OPEN BIFAST");
				$('#form_result').html(data);
				if(window.confirm("Apakah ibbiz dibuka juga?")){
					window.location.href = "../ibbiz/show_bifast";
				}
			}
		});
