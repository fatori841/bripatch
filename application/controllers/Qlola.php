...
class Qlola extends CI_Controller {

	function __construct(){
		parent::__construct();
		$this->load->model('Menu_model');
		//$this->load->model('Qlola_model');
	...
	public function index()
	{
		if (!$this->session->has_userdata('ibo_is_login')) {
			...
			$data_menu = $this->Menu_model->get_menu_apps(68);
	...
	public function capture_micro()
	{
		...
		$arr_data = array();
		$data['sidemenu'] = $data_menu_result;
		$data['isi'] = $this->load->view('qlola/micro/capture_micro', $arr_data, true);
	...
	{		
		...
		$data_micro = array('namespace'=>'wsa');
		$data_return['micro_list'] = $this->Micro_model->get_capture_micro($data_micro);
		
		echo $this->load->view('qlola/micro/capture_micro_result', $data_return, true);
	}
	...
	public function compare_micro()
	{
		...
		$arr_data = array();
		$data['sidemenu'] = $data_menu_result;
		$data['isi'] = $this->load->view('qlola/micro/compare_micro', $arr_data, true);
	...
	{		
		...
		$data_micro = array('capture_status'=>0, 'namespace'=>'wsa', 'deployment'=>$datapost['deployment_txt']);
		$data_return['micro_list'] = $this->Micro_model->get_capture_micro($data_micro);
		
		echo $this->load->view('qlola/micro/capture_micro_compare_result', $data_return, true);
	}
